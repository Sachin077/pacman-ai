    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                result[newPos] += prob * self.beliefs[oldPos]
        self.beliefs = result
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        numPos = len(pos)
        self.particles = [pos[i%numPos] for i in xrange(self.numParticles)]
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if noisyDistance is None:
            self.particles = [self.getJailPosition() for x in xrange(len(self.particles))]
        else:
            ...
            if weights.totalCount() == 0:
                ...
                self.particles = util.nSample(weights.values(), weights.keys(), self.numParticles)
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            particle_pos.add(p)
        distributions = dict()
        for oldPos in particle_pos:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            distributions[oldPos] = newPosDist
        for oldPos in self.particles:
            new_particles.append(util.sample(distributions[oldPos]))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        args = [self.legalPositions for i in range(self.numGhosts)]
        perm_iter = itertools.product(*args)
        perm_lst = [perm_iter.next() for i in range(pow(len(self.legalPositions), self.numGhosts))]
        random.shuffle(perm_lst)
        n = len(perm_lst)
        self.particles = [perm_lst[i % n] for i in range(self.numParticles)]  
    ...
    def observeState(self, gameState):
        ...
        weights = belief.copy()
        for i in range(self.numGhosts):
            noisy = noisyDistances[i]
            model = emissionModels[i]
            if noisy != None:
                for p in belief:
                    weights[p]*=model[util.manhattanDistance(p[i], pacmanPosition)]
        if weights.totalCount()==0:
            self.initializeParticles()
        else:
            weights.normalize()
