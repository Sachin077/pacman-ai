...
def enhancedFeatureExtractorDigit(datum):
    ...
    for y in range(DIGIT_DATUM_HEIGHT):
        ...
        count = 0
        for x in range(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x, y) != curr:
                curr = 1- curr
                switch += 1
        if switch > 1:
            features["hswtich_{0}_0".format(y)]=1
        else:
            features["hswtich_{0}_0".format(y)]=0
        if switch > 5:
            features["hswtich_{0}_1".format(y)]=1
        else:
            features["hswtich_{0}_1".format(y)]=0
    return features
