    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[new_x][new_y]:
                new_state = [(new_x, new_y), corner1, corner2, corner3, corner4]
                if (new_x, new_y) in self.corners:
                    new_state[self.corners.index((new_x, new_y))+1] = True
...
def cornersHeuristic(state, problem):
    ...
    unreached_corners = [corners[i-1] for i in range(1, 5) if not state[i]]
    xy = state[0]
    for c in unreached_corners:
        dist.append(util.manhattanDistance(xy, c))
...
def foodHeuristic(state, problem):
    ...
    for x in range(foodGrid.width):
        for y in range(foodGrid.height):
            if foodGrid[x][y]:
                dist.append(mazeDistance((x, y), position, problem.startingGameState))
    return max(dist)
